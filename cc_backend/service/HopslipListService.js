    ...
    pslipList: async (req, res) => {
        ...
        try {
            ...
            const result = await connection.execute(
                `BEGIN                        
                CCTECHAGRI.cms_cc_pkg.Ho_Pslip_Data (:p_login_cd,:p_Ho_Pslip_Details,:P_Status_id,:P_Status_Message);
                END;`,
                {
                   ...
                   // P_ho_no : { val: 'result', dir: oracledb.BIND_IN ,val: null},
                    p_Ho_Pslip_Details:{ type: oracledb.CURSOR, dir : oracledb.BIND_OUT },
            ...
            const resultSet1 = result.outBinds.p_Ho_Pslip_Details;
           ...
           const plsiList = await self.parseObject(oraColumns, rows1);
            let { P_Login_Cd, P_Login_Name, P_Status_id, P_Status_Message } = jsonObj;
            const response = { id: id,PslipHoList: plsiList, user: P_Login_Name, status: P_Status_id, P_Status_Message};
