...
const self=module.exports = {
    hoDetails: async (req, res,next) => {
        ...
        try {
            ...
            console.log('Connection pool started verifyService');
            ...
            const result = await connection.execute(
                ...
                END;`,
                {
                    P_Rfid_Tag:   { type: oracledb.VARCHAR2, dir: oracledb.BIND_IN,
                        val: id.id.toString() },//'E2801170200010967CE309FD',  // Bind type is determined from the data.  Default direction is BIND_IN
           ...
           // console.log(JSON.stringify(rows1));
            ...
            console.log('Connection pool error verifyService'+err);
            console.error(err);
        } finally {
            if (connection) {
                try {
    ...
    parseObject:async(columns,rows)=>{
       ...
       // console.log('rows',rows);
        ...
        for (let index = 0; index < rows.length; index++) {   
            let i=0;  
            col.forEach(element => {
                if(element.toString()==="RFID_TAG")
                {
                    let val={}
                    let strSplit=rows[index].toString().split(',');
                    val[element]=strSplit[2];
